...
class MyDataset(Dataset):
    ...
    def __init__(
        ...
        if cuda.device_count() >= 2 and not keep_in_memory:
            raise ValueError(
                f"多GPU时使用MyDataset，参数keep_in_memory必须=True，否则无法进行分布式训练. 当前keep_in_memory={keep_in_memory}"
            )
    ...
    def item_generator(
        ...
        while True:
            ...
            for prompt, response in zip(
                parquet_table["prompt"], parquet_table["response"]
            ):
    ...
    def collate_fn(self, data: list[list]) -> dict:
        ...
        prompt = tokenizer(
            [item[0] for item in data], padding=True, return_token_type_ids=False
        )
        response = tokenizer(
            [item[1] for item in data], padding=True, return_token_type_ids=False
        )
...
class ParquetDataset:
    ...
    def __init__(
        ...
        dataset = load_dataset(
            "parquet", data_files=parquet_file, cache_dir=cache_dir, streaming=streaming
        )

        # 这里的batch_size不是训练的batch_size，是传递给precess_batch_func批处理的batch_size
        dataset = dataset.map(
            self.precess_batch_func,
            batched=True,
    ...
    def collate_fn(self, data: list[list]) -> dict:
        ...
        response_encoded = tokenizer(
            response, padding=True, return_token_type_ids=False
        )
    ...
    def __len__(self) -> int:
        ...
        if len(self.len_dict) == 1:
            ...
            raise Exception(
                "this dataset contains many splited datasets, use `get_dataset_size(split_name)` function to get length, e.g: get_dataset_size('train')"
            )
...
if __name__ == "__main__":
    ...
    # example 1：
    dataset = MyDataset(
        parquet_file, tokenizer_dir, keep_in_memory=False, max_seq_len=128
    )
    ...
    dataset = ParquetDataset(
        parquet_file, tokenizer_dir, keep_in_memory=True, max_len=32
    )
    dataloader = DataLoader(
        dataset["train"], batch_size=32, collate_fn=dataset.collate_fn
    )
