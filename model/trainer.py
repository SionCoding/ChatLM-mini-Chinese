...
class ChatTrainer:
    def __init__(
        ...
        self.logger = Logger(
            "chat_trainer", std_out=True, save2file=True, file_name=None
        )
        ...
        if self.accelerator and self.model:
            ...
            if ins.lower() in ("yes", "y"):
                ...
                self.accelerator.print(
                    "model ckeck point has been saved in {}".format(
                        self.train_config.train_state_dir
                    )
                )
    ...
    def delete_early_checkpoint(
        ...
        model_save_path = "/".join(
            model_save_path.split("/")[0:-1]
        )  # 删除末尾文件名后缀

        model_files = get_path_of_suffix_files(
            model_save_path, suffix=".bin", with_create_time=True
        )
    ...
    def train(self, is_keep_training: bool = False, is_finetune: bool = False) -> None:
        ...
        keep_in_memory = (
            True if unuse_mem >= 48.0 or torch.cuda.device_count() >= 2 else False
        )

        if accelerator.is_main_process:
            log.info(
                "cpu memory available: {:.2f} GB, disk space available: {:.2f} GB, keep dataset in memory: {}.".format(
                    unuse_mem, unuse_disk, keep_in_memory
                ),
                save_to_file=True,
            )
            log.info(
                "operation: {}, keep training: {}, loading datasets ...".format(
                    "finetune" if is_finetune else "train", is_keep_training
                )
            )
        ...
        if accelerator.is_main_process:
            log.info(
                "train dataset size: {}, steps per epoch:{}; validation dataset size: {}, steps per validation: {}; datalodater num_workers: {}.".format(
                    len(train_dataset),
                    steps_per_epoch,
                    len(valid_dataset),
                    eval_steps,
        ...
        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(
            ...
            steps_per_epoch=int(
                np.ceil(len(train_dataset) / (batch_size * accumulation_steps))
            ),  # 梯度累积相当于增大了batch_size
        ...
        if accelerator.is_main_process:
            ...
            epoch_progress = progress.add_task(
                description="epoch: ", show_info="", total=train_config.epochs
            )
            steps_progress = progress.add_task(
                description="steps: ",
                show_info="",
        ...
        for epoch in range(train_config.epochs):
            ...
            for step, batch_data in enumerate(train_dataloader):
                ...
                if step % logging_steps == 0 or step == steps_per_epoch:
                    ...
                    info_txt = (
                        "training loss: epoch:{}, step:{}, loss:{}, device:{}".format(
                            epoch, step, loss_cpu, str(accelerator.device)
                        )
                    )

                    log.info(
                        info_txt, std_out=False, save_to_file=True
                    )  # 保存 loss 到文件
            ...
            if accelerator.is_main_process:
                ...
                info_txt = "epoch log: epoch:{}, avg_loss:{}, cur_bleu4:{}, best_bleu4:{}, best_epoch:{}".format(
                    epoch,
                    my_average(epoch_loss_list),
                    cur_bleu4_score,
                    best_bleu4,
    ...
    def evaluate(
            ...
            for step, batch_data in enumerate(valid_dataloader):
                ...
                if accelerator.is_main_process:
                    ...
                    self.progress.update(
                        self.eval_progress,
                        show_info="step: {}/{}".format(step, eval_steps),
                    )
                ...
                target_ids = (
                    accelerator.gather_for_metrics(target_ids).detach().cpu().numpy()
                )
                ...
                bleu4_scores = [
                    get_bleu4_score(reference=target_ids[i], outputs=outputs[i])
        ...
        if accelerator.is_main_process:
            self.progress.update(
                self.eval_progress, show_info="bleu4 score: {}".format(avg_bleu4_score)
            )
    ...
    def test(self, best_epoch: int = 0) -> None:
        ...
        if accelerator.is_main_process:
            ...
            steps_progress = progress.add_task(
                description="steps: ", show_info="", total=steps
            )
            progress.start()
        ...
        with torch.no_grad():
            for step, batch_data in enumerate(test_dataloader):
                ...
                if accelerator.is_main_process:
                    progress.advance(steps_progress, advance=1)
                    progress.update(
                        steps_progress, show_info="step: {}/{}".format(step, steps)
                    )
        ...
        if accelerator.is_main_process:
            progress.update(
                steps_progress, show_info="bleu4 score: {}".format(avg_bleu4_score)
            )

        info_txt = "test_dataset_size: {}, avg_bleu4_score:{}.".format(
            len(test_dataset), avg_bleu4_score
        )
