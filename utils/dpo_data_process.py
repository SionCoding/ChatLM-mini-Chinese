...
def process_alpaca_gpt4_data(max_len: int = 512) -> None:
    ...
    with open(read_file, "r", encoding="utf-8") as f:
        ...
        for item in progress.track(data):
            ...
            my_data.append({"prompt": prompt, "chosen": response})
...
def generate_alpaca_gpt4_reject_response(
    groups_cnt: int = 50000, max_len: int = 320, batch_size: int = 32
) -> None:
    """生成不是很满意的回答回答"""
    print("load model...")
    ...
    log.info(
        "length of {} is {}".format(save_rw_json_file, len(data)), save_to_file=True
    )
    ...
    for i, item in progress.track(enumerate(data), total=len(data)):
        ...
        if len(batch_prompt) >= batch_size or i == len(data) - 1:
            ...
            encoded = tokenizer.batch_encode_plus(
                batch_prompt, truncation=False, padding=True
            )
        ...
        if len(model_outs) % 2000 == 0:
            ...
            try:
                with open(
                    PROJECT_ROOT + "/data/outs.ckp.json", "w", encoding="utf-8"
                ) as f:
...
def merge_rlhf_data(max_len: int = 512) -> None:
    ...
    for read_file in read_files:
        ...
        for prompt, chosen, rejected in progress.track(
            zip(pf["prompt"], pf["chosen"], pf["rejected"]), total=pf.num_rows
        ):
            ...
            if (
                len(prompt) > max_len
                or len(chosen) > max_len
                or len(rejected) > max_len
            ):
                continue
            ...
            if (
                ...
                or len(chosen) == 0
                or len(rejected) == 0
                or rejected.strip() == chosen.strip()
            ):
                continue
...
def split_train_eval_dataset() -> None:
    ...
    log.info(
        "train size: {}, eval size:{}".format(len(train_data), len(eval_data)),
        save_to_file=True,
    )
