...
def remove_duplicate_punctuation(sentence: str) -> str:
    ...
    while p < n:
        ...
        while (
            p + 1 < n and sentence[p] in punctuation and sentence[p + 1] in punctuation
        ):
            p += 1
        p += 1
...
def read_and_write_template(
    read_file: str, write_to_file: str, call_back: object, group_cnt: int = 10000
) -> None:
    """
    ...
    log.info(
        "原始文件:{}，共{}行，处理后剩余{}行，保存到文件：{}。耗时：{:.6}s".format(
            read_file, raw_line_cnt, keep_line_cnt, write_to_file, end - start
        ),
        save_to_file=True,
    )
...
def repair_line_error_csv_file(
    raw_csv_file: str,
    save_suffix: str,
    ...
    with codecs.open(raw_csv_file, "r", encoding=read_encoding, errors="ignore") as f:
        ...
        with open(
            raw_csv_file[:-4] + save_suffix, "w", encoding="utf-8", newline=""
        ) as f:
            writer = csv.writer(f)
...
def process_chinese_medical_datasets(response_less_word: int = 15) -> None:
    ...
    need_to_repair_files = [
        ...
        and file_name[0:-4] + suffix not in raw_data_files
    ]
...
def process_finace_dataset(
    prompt_less_word: int = 10, response_less_word: int = 15
) -> None:
    """
    处理金融问答数据集
    """
    ...
    if not exists(finace_data_file[0:-4] + suffix):
        repair_line_error_csv_file(
            finace_data_file, save_suffix=suffix, read_encoding="utf-8"
        )
    ...
    def process_function(line: str) -> dict:
        ...
        if (
            len(item[0]) + len(item[1]) < prompt_less_word
            or len(item[2]) < response_less_word
        ):
            return None
    ...
    write_file = (
        PROJECT_ROOT
        + "/data/my_data/"
        + read_file.split("/")[-1][0 : -(len(suffix))]
        + ".parquet"
    )
...
def process_zhihu_kol_dataset(
    prompt_less_word: int = 4, response_less_word: int = 10, group_cnt: int = 10000
) -> None:
    """
    处理知乎数据集
    ...
    """
    raw_zhihu_data_path = (
        abspath(dirname(dirname(__file__))) + "/data/raw_data/zhihu-kol"
    )
    file_names = []
    suffix = ".parquet"
    ...
    for file in file_names:
        ...
        for prompt, response in progress.track(
            zip(pf["INSTRUCTION"], pf["RESPONSE"]), total=pf.num_rows
        ):
            all_cnt += 1
    ...
    log.info(
        "save file to: {}, 全部数据共{}行，清洗后剩余{}行".format(
            save_file, all_cnt, keep_cnt
        ),
        save_to_file=True,
    )


def process_belle_knowledge_enhanced_dataset(
    response_less_words: int = 15, group_cnt: int = 10000
) -> None:
    """
    处理belle开源的知识增强数据集
    """
...
def process_zh_wiki_data_to_datset(
    groups_cnt: int = 10000, max_len: int = 512, seed: int = 23333
) -> None:
    """
    将Wiki中文数转换为问答数据集
    ...
    log.info(
        "merge into file: {}, 全部数据共{}行，清洗后剩余{}行".format(
            zhwiki_simple_file, all_cnt, keep_cnt
        ),
        save_to_file=True,
    )
...
def merge_dataset_as_single_file(
    groups_cnt: int = 50000,
    max_len: int = 512,
    min_len: int = 3,
    cut_max_len: bool = False,
) -> None:
    """
    将多个数据集合并为一个数据集
    """
    from_parquet_files = get_path_of_suffix_files(
        PROJECT_ROOT + "/data/my_data", ".parquet"
    )
...
def remove_dataset_duplicate_rows(groups_cnt: int = 50000) -> None:
    ...
    log.info(
        "merge into file: {}, 全部数据共{}行，文档去重后剩余{}行".format(
            save_file, all_cnt, keep_cnt
        ),
        save_to_file=True,
    )
...
def shuffle_parquet_dataset(
    parquet_file: str, shuffle_file: str, seed: int = 23333, groups_cnt: int = 65536
) -> None:
    """
    打乱一个parquet文件数据集
    """
...
def count_my_parquet_data(parquet_file: str = None) -> None:
    ...
    if not parquet_file:
        my_data_files = get_path_of_suffix_files(
            PROJECT_ROOT + "/data/my_data", ".parquet"
        )
...
def split_train_valid_test_datasets(
    ...
    if exists(train_parquet_file):
        assert delete_file(train_parquet_file)
    if exists(test_parquet_file):
        assert delete_file(test_parquet_file)
    if exists(valid_parquet_file):
        assert delete_file(valid_parquet_file)
    ...
    for prompt, response in progress.track(
        ...
        cur_data = {
            "prompt": "".join(prompt[0:max_len]),
            "response": "".join(response[0:max_len]),
        }
...
def dataset_length_cnt() -> None:
    ...
    plt.subplot(2, 2, 1)
    plot_sub_bar(
        plt, que_pd["length"], que_pd["count"], title="prompt length", color="c"
    )

    plt.subplot(2, 2, 2)
    plot_sub_bar(
        plt, ans_pd["length"], ans_pd["count"], title="response length", color="g"
    )
...
def process_belle_knowledge_enhanced_dataset_for_finetune(
    max_len: int = 320, group_cnt: int = 50000
) -> None:
    """
    处理belle开源的知识增强数据集
    """
